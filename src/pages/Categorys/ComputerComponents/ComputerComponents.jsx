...
const ComputerComponents = () => {
    ...
    return (
        <div>
            ...
            <h2 className="text-2xl md:text-4xl font-semibold mt-2 text-[#f16022] flex justify-center gap-2 items-center">Computer Components <FiChevronsRight></FiChevronsRight> </h2>
            <hr className="h-1 mt-2 bg-[#f16022] border-none rounded-xl mb-7" />
            <div className="grid gap-5 grid-cols-1 md:grid-cols-2 lg:grid-cols-4 ">
                {
                    product?.map(products => <ComputerComponentsCard key={products._id} products={products}></ComputerComponentsCard>)
                }
            </div>
        </div>
    );
